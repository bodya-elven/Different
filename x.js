(function () {
    ...
    var PORNO365_DOMAIN = 'https://w.porno365.gold',
        LENKINO_DOMAIN = 'https://mob.lenkino.love',
        LONGVIDEOS_DOMAIN = 'https://www.longvideos.xxx',
        PORNHUB_DOMAIN = 'https://rt.pornhub.com',
        PORNDISH_DOMAIN = 'https://www.porndish.com',
        YOUPERV_DOMAIN = 'https://youperv.com',
        PORNMZ_DOMAIN = 'https://pornmz.net';
    ...
    function startPlugin() {
        ...
        function smartRequest(url, onSuccess, onError, customHeaders) { 
        ...
        Adapters['porno365'] = {
                ...
                if (stext) { 
                    ...
                    for (var d = 0; d < divs.length; d++) { 
                        ...
                        for (var l = 0; l < links.length; l++) { 
                            ...
                            var isActive = (!span && (divs[d].classList.contains('active_sort') || divs[d].classList.contains('current_sort'))) || (parentLi && (parentLi.classList.contains('active_sort') || parentLi.classList.contains('current_sort'))) || links[l].classList.contains('active_sort') || links[l].classList.contains('current_sort');
                            if (isActive) { activeTitle = fullTitle; items.push({ title: '⇅ ' + fullTitle, action: 'none' }); } 
            ...
            getMenu: function(doc) { 
                ...
                m.push({ title: 'Категорії', action: 'cats_custom', sel: '.video-categories:not(.video-models) a' }, { title: 'Теги', action: 'cats_custom', sel: '.video-tags a' }, { title: 'Схожі відео', action: 'sim' }); 
                return m; 
            },
        ...
        Adapters['lenkino'] = {
            ...
            parse: function(doc, obj) {
                ...
                if (obj.is_studios || (obj.url && obj.url.indexOf('/channels') !== -1)) {
                    ...
                    if (contS) {
                        var stus = contS.querySelectorAll('.item');
                        for (var s = 0; s < stus.length; s++) {
                            var lS = stus[s].querySelector('a.len_pucl'), iS = stus[s].querySelector('img'), tS = stus[s].querySelector('.itm-tit'), noImg = stus[s].querySelector('.no-img');
                            if (lS) {
            ...
            getMenu: function(doc) {
                var m = [], added = [], mEls = doc.querySelectorAll('.itm-opt-mdl, a[href*="/pornstar/"]');
                for (var i = 0; i < mEls.length; i++) {
                    var t = mEls[i].textContent.trim(), h = mEls[i].getAttribute('href');
                    if (t && h && added.indexOf(t) === -1 && t.length > 1) { m.push({ title: t, action: 'direct', url: h }); added.push(t); }
                }
                var sEls = doc.querySelectorAll('a[href*="/channel/"]');
                for (var j = 0; j < sEls.length; j++) {
                    var ts = sEls[j].textContent.trim(), hs = sEls[j].getAttribute('href');
                    if (ts && hs && added.indexOf(ts) === -1) { m.push({ title: ts, action: 'direct', url: hs }); added.push(ts); }
                }
            ...
            play: function(doc, el) {
                ...
                if (u && u[1]) str.push({ title: 'SD', url: u[1] });
                if (a && a[1]) str.push({ title: 'HD', url: a[1] });
                if (str.length > 0) {
                    var pData = { title: el.name, url: str[str.length-1].url, quality: str, headers: { 'Referer': this.domain + '/', 'Origin': this.domain } };
                    Lampa.Player.play(pData); Lampa.Player.playlist([pData]);
        ...
        Adapters['longvideos'] = {
            ...
            parse: function(doc, obj) {
                ...
                if (obj.is_models || path === '/models') { 
                    var resM = [], mEls = doc.querySelectorAll('.list-models .item'); 
                    for(var m=0; m<mEls.length; m++) { 
                        if(mEls[m].querySelector('.no-thumb')) continue; 
                        var iM = mEls[m].querySelector('img'); 
                        if(!iM) continue; 
                        var sM = iM.getAttribute('data-original')||iM.getAttribute('data-src')||iM.getAttribute('src'); 
                        if(sM && sM.indexOf('data:image')===0) sM = iM.getAttribute('data-src')||iM.getAttribute('data-original'); 
                        if(sM && sM.indexOf('//')===0) sM='https:'+sM; else if(sM && sM.indexOf('/')===0) sM=this.domain+sM; 
                        var lM = mEls[m].tagName==='A'?mEls[m]:(mEls[m].querySelector('a')||mEls[m]), nM = iM.getAttribute('alt')||lM.getAttribute('title')||'', tM = mEls[m].querySelector('.title, .name, h5'); 
                        if(!nM && tM) nM = tM.textContent.trim(); 
                        var cM = mEls[m].querySelector('.videos'); 
                        if(nM && sM) resM.push({name: formatTitle(nM, cM?cM.textContent.trim():'', '☰'), url: this.domain+lM.getAttribute('href'), picture: sM, is_grid: true, is_models_grid: true}); 
                    } 
                    return resM; 
                }
                if (obj.is_studios || path === '/sites') { 
                    var resS = [], sEls = doc.querySelectorAll('#list_content_sources_sponsors_list_items .headline'); 
                    for(var s=0; s<sEls.length; s++) { 
                        var lS = sEls[s].querySelector('a.more')||sEls[s].querySelector('a'), tS = sEls[s].querySelector('h1, h2, h3, h4, .title')||lS; 
                        if(lS) { 
                            var nS = tS.textContent.trim(), sp = tS.querySelector('span'); 
                            if(sp) nS = nS.replace(sp.textContent.trim(), '').trim(); 
                            if(nS) resS.push({name: nS, url: this.domain+lS.getAttribute('href'), is_grid: true, is_studios_noimg: true}); 
                        } 
                    } 
                    return resS; 
                }
                if (obj.is_categories || obj.is_trends || path === '/categories') { 
                    var resC = [], cEls = doc.querySelectorAll(obj.is_trends ? '.tags__item' : '.list-categories__row--list a'); 
                    for(var c=0; c<cEls.length; c++) { 
                        var nC = cEls[c].textContent.trim(), hC = cEls[c].getAttribute('href'); 
                        if(nC && hC) resC.push({name: nC, url: hC.startsWith('http')?hC:this.domain+hC, is_grid: true}); 
                    } 
                    return resC; 
                }
                var res = [], sel = obj.is_related ? '.related-videos .item' : '.list-videos .item, .item', els = doc.querySelectorAll(sel); 
                for(var i=0; i<els.length; i++) { 
                    var l = els[i].querySelector('a.thumb_title'); 
                    if(!l) continue; 
                    var n = l.textContent.trim(), imgEl = els[i].querySelector('img.thumb'), img = imgEl ? (imgEl.getAttribute('data-src')||imgEl.getAttribute('src')) : ''; 
                    if(img && img.indexOf('//')===0) img='https:'+img; else if(img && img.indexOf('/')===0) img=this.domain+img; 
                    var pEl = els[i].querySelector('.img.thumb__img'), pUrl = pEl ? pEl.getAttribute('data-preview') : ''; 
                    if(pUrl && pUrl.indexOf('//')===0) pUrl='https:'+pUrl; 
                    var tEl = els[i].querySelector('.duration'); 
                    res.push({name: formatTitle(n, tEl?tEl.textContent.replace(/Full Video/gi,'').trim():'', '▶'), url: this.domain+l.getAttribute('href'), picture: img, preview: pUrl}); 
                } 
                return res;
            },
            ...
            getSort: function(doc, curUrl) { 
                var act = 'Latest', items = [], list = doc.querySelector('#_sort_list, #list_videos_common_videos_list_sort_list, #list_content_sources_sponsors_list_sort_list'); 
                var pfx = curUrl.indexOf('/top-rated')!==-1 ? 'Top Rated - ' : (curUrl.indexOf('/most-popular')!==-1 ? 'Most Viewed - ' : ''); 
                if (list) { 
                    var links = list.querySelectorAll('a'); 
                    for (var i=0; i<links.length; i++) items.push({title: pfx + links[i].textContent.trim(), url: this.domain + links[i].getAttribute('href')}); 
                    var sSt = doc.querySelector('.sort strong, .filter-channels strong'); 
                    if(sSt) { 
                        var rT = sSt.textContent.trim(), iSp = sSt.querySelector('span, small'); 
                        if(iSp) rT = rT.replace(iSp.textContent.trim(), '').trim(); 
                        act = pfx + rT; 
                    } 
                } else if (curUrl.indexOf('/models')===-1 && curUrl.indexOf('/sites')===-1 && curUrl.indexOf('/search')===-1) { 
                    if(pfx==='Top Rated - ') act = 'Top Rated'; else if(pfx==='Most Viewed - ') act = 'Most Viewed'; 
                    items.push({title: 'Latest', url: this.domain+'/latest-updates/'}, {title: 'Top Rated', url: this.domain+'/top-rated/all/'}, {title: 'Most Viewed', url: this.domain+'/most-popular/all/'}); 
                } 
                return { active: act, items: items }; 
            },
            getMenu: function(doc) { 
                var m = [], src = doc.querySelectorAll('video source'); 
                if (src.length > 1) m.push({ title: 'Відтворити в ' + (src[1].getAttribute('label') || 'SD'), action: 'play_direct', url: src[1].getAttribute('src') }); 
                var mods = doc.querySelectorAll('.btn_model'), added = []; 
                for (var i = 0; i < mods.length; i++) { 
                    var t = mods[i].textContent.trim(), u = mods[i].getAttribute('href'); 
                    if (t && u && added.indexOf(t) === -1) { m.push({ title: t, action: 'direct', url: u }); added.push(t); } 
                } 
                var spon = doc.querySelectorAll('.btn_sponsor, .btn_sponsor_group'); 
                for (var j = 0; j < spon.length; j++) { 
                    var st = spon[j].textContent.trim(); 
                    if(spon[j].classList.contains('btn_sponsor_group')) st += ' (Network)'; 
                    m.push({ title: st, action: 'direct', url: spon[j].getAttribute('href') }); 
                } 
                m.push({ title: 'Категорії', action: 'cats_custom', sel: '.btn_tag, .btn_tag.hidden' }, { title: 'Схожі відео', action: 'sim' }); 
                return m; 
            },
            play: function(doc, el) { 
                var str = [], src = doc.querySelectorAll('video source'); 
                for(var i=0;i<src.length;i++) str.push({title: src[i].getAttribute('label')||'Оригінал', url: src[i].getAttribute('src')}); 
                if(str.length>0) { 
                    var pData = { title: el.name, url: str[0].url, quality: str };
        ...
        Adapters['pornhub'] = {
            ...
            parse: function(doc, obj) {
                ...
                if (path.indexOf('/categories') !== -1) { 
                    var resCat = [], catEls = doc.querySelectorAll('#categoriesListSection li.tagColumn'); 
                    for(var c=0; c<catEls.length; c++) { 
                        var lC = catEls[c].querySelector('a'), iC = catEls[c].querySelector('img'); 
                        if(lC && iC) { 
                            var nC = catEls[c].querySelector('.category-title') ? catEls[c].querySelector('.category-title').textContent.trim() : iC.getAttribute('alt'), hC = lC.getAttribute('href'); 
                            if(nC) resCat.push({name: nC, url: hC.startsWith('http')?hC:this.domain+hC, picture: iC.getAttribute('src'), is_grid: true}); 
                        } 
                    } 
                    return resCat; 
                }
                if (path === '/pornstars' || path === '/channels' || obj.is_models) { 
                    var isSt = path === '/channels', resM = [], sel = isSt ? '.channelsList li' : '#pornstarListSection li', mEls = doc.querySelectorAll(sel); 
                    for(var m=0; m<mEls.length; m++) { 
                        var lM = mEls[m].querySelector('a'), iM = mEls[m].querySelector('img'), tM = mEls[m].querySelector('.title'); 
                        if(lM && iM) { 
                            var nM = tM ? tM.textContent.trim() : iM.getAttribute('alt'); 
                            if(nM) resM.push({name: formatTitle(nM, '', '☰'), url: this.domain+lM.getAttribute('href'), picture: iM.getAttribute('src'), is_grid: true, is_models_grid: !isSt, is_studios_noimg: isSt}); 
                        } 
                    } 
                    return resM; 
                }
                var res = [], elements = doc.querySelectorAll('li.videoblock, li.pcVideoListItem'); 
                for (var i = 0; i < elements.length; i++) { 
                    ...
                    if (linkEl && imgEl) { 
                        var name = imgEl.getAttribute('title') || imgEl.getAttribute('alt') || linkEl.textContent.trim(), vUrl = linkEl.getAttribute('href'); 
                        if (vUrl && vUrl.indexOf('javascript') === -1) { 
                            ...
                            res.push({ name: formatTitle(name, time, '▶'), url: this.domain + (vUrl.startsWith('/')?'':'/') + vUrl.replace(/^\//,''), picture: img, preview: pUrl }); 
                        } 
                    } 
                } 
                return res;
            ...
            getSort: function(doc, curUrl) { 
                var uMatch = curUrl.match(/c=([^&]+)/), currentC = uMatch ? uMatch[1] : '', oMatch = curUrl.match(/o=([^&]+)/), currentO = oMatch ? oMatch[1] : '', active = 'Новые', items = []; 
                for (var st = 0; st < PH_SORTS.length; st++) { if (PH_SORTS[st].val === currentO || (currentO === '' && PH_SORTS[st].val === 'cm')) active = PH_SORTS[st].title; } 
                items.push({ title: '⇅ ' + active, action: 'none' }); 
                for (var st2 = 0; st2 < PH_SORTS.length; st2++) { 
                    if (PH_SORTS[st2].title === active) continue; 
                    var nUrl = this.domain + '/video', params = []; 
                    if (currentC) params.push('c=' + currentC); 
                    if (PH_SORTS[st2].val) params.push('o=' + PH_SORTS[st2].val); 
                    if (params.length > 0) nUrl += '?' + params.join('&'); 
                    items.push({ title: PH_SORTS[st2].title, url: nUrl }); 
                } 
                return { active: active, items: items }; 
            },
            getMenu: function(doc) { 
                var m = [], match = doc.documentElement.innerHTML.match(/var\s+flashvars_\d+\s*=\s*(\{[\s\S]+?\});/); 
                if (match) { 
                    try { 
                        ...
                        if (fv.mediaDefinitions) { 
                            ...
                            if(defs.length>1) m.push({title: 'Відтворити в '+(defs[1].quality||'SD'), action: 'play_direct', url: defs[1].videoUrl.replace(/\\/g, ''), headers: {'Referer': this.domain+'/', 'Origin': this.domain} }); 
                        } 
                    } catch(e){} 
                } 
                var phMods = doc.querySelectorAll('.pornstarsWrapper .pstar-list-btn'); 
                for (var p = 0; p < phMods.length; p++) m.push({ title: phMods[p].textContent.trim(), action: 'direct', url: this.domain + phMods[p].getAttribute('href') }); 
                m.push({ title: 'Схожі відео', action: 'sim' }); 
                return m; 
            },
            play: function(doc, el) { 
                var match = doc.documentElement.innerHTML.match(/var\s+flashvars_\d+\s*=\s*(\{[\s\S]+?\});/), str = []; 
                if (match) { 
                    try { 
                        var fv = JSON.parse(match[1]); 
                        if (fv.mediaDefinitions) { 
                            ...
                            for(var k=0; k<defs.length; k++) { 
                                ...
                                str.push({ title: qTitle, url: defs[k].videoUrl.replace(/\\/g, '') }); 
                            } 
                        } 
                    } catch(e) {} 
                } 
                if (str.length > 0) { 
                    var pData = { title: el.name, url: str[0].url, quality: str, headers: { 'Referer': this.domain+'/', 'Origin': this.domain } }; 
                    Lampa.Player.play(pData); Lampa.Player.playlist([pData]); 
                } 
            }
        };
        ...
        Adapters['porndish'] = {
            ...
            parse: function(doc, obj) {
                var path = (obj.url || '').replace(this.domain, '').split('?')[0];
                if (path === '/categories' || obj.is_categories) { 
                    var resCat = [], cats = doc.querySelectorAll('.g1-nav-menu li a, .entry-category'); 
                    for(var c=0; c<cats.length; c++) { 
                        var tC = cats[c].textContent.trim(), hC = cats[c].getAttribute('href'); 
                        if(tC && hC) resCat.push({name: tC, url: hC.startsWith('http')?hC:this.domain+hC, is_grid: true}); 
                    } 
                    return resCat; 
                }
                var res = [], elements = doc.querySelectorAll('article.post, .g1-collection-item article, .g1-featured-item article, .g1-mosaic-item article, article.entry-tpl-tile, article.entry-tpl-grid'); 
                for (var i = 0; i < elements.length; i++) { 
                    var el = elements[i], linkEl = el.querySelector('.entry-title a') || el.querySelector('a.g1-frame'), titleEl = el.querySelector('.entry-title a'), imgEl = el.querySelector('img'), timeEl = el.querySelector('.mace-video-duration'); 
                    if (linkEl) { 
                        var vUrl = linkEl.getAttribute('href'); 
                        if(vUrl && vUrl.indexOf('http')!==0) vUrl = this.domain+(vUrl.startsWith('/')?'':'/')+vUrl.replace(/^\//,''); 
                        var name = titleEl ? titleEl.textContent.trim() : (imgEl?imgEl.getAttribute('alt'):linkEl.textContent.trim()), time = timeEl ? timeEl.textContent.trim() : ''; 
                        if (name && vUrl) res.push({ name: formatTitle(name, time, '▶'), url: vUrl, is_porndish_list: true }); 
                    } 
                } 
                return res;
            },
            ...
            play: function(doc, el) { 
                var streamix = doc.documentElement.innerHTML.match(/streamixContent.*?src=\\?["']([^"'\s>]+)\\?["']/i); 
                if (streamix) { 
                    var rawUrl = streamix[1].replace(/\\/g, ''), parts = rawUrl.match(/https?:\/\/([^\/]+)\/.*?(?:e|v|embed|d)\/([a-zA-Z0-9_-]+)/i); 
                    if (parts) { 
                        smartRequest('https://' + parts[1] + '/api/stream?filecode=' + parts[2], function(j) { 
                            try { 
                                var jd = JSON.parse(j); 
                                if (jd.streaming_url) { 
                                    var pData = { title: el.name, url: jd.streaming_url };
                                    Lampa.Player.play(pData); 
                                    Lampa.Player.playlist([pData]); 
                                } 
                            } catch(e){} 
                        }, null, { "Referer": "https://" + parts[1] + "/" }); 
                        return; 
                    } 
                }
        ...
        Adapters['youperv'] = {
            ...
            parse: function(doc, obj) { 
                var res = [], blocks = doc.querySelectorAll('.items.clearfix'), container = obj.is_related ? (blocks.length > 0 ? blocks[blocks.length - 1] : doc) : doc, elements = container.querySelectorAll('.item'); 
                for (var i = 0; i < elements.length; i++) { 
                    var el = elements[i]; 
                    ...
                    if (linkEl) { 
                        var vUrl = this.domain + (linkEl.getAttribute('href').startsWith('/') ? '' : '/') + linkEl.getAttribute('href').replace(/^\//, ''), img = imgEl ? (imgEl.getAttribute('data-src') || imgEl.getAttribute('src') || '') : ''; 
                        if (img && img.indexOf('//') === 0) img = 'https:' + img; else if (img && img.indexOf('/') === 0) img = this.domain + img; 
                        ...
                        var time = timeEl ? timeEl.textContent.trim() : ''; 
                        if (name && vUrl) res.push({ name: formatTitle(name, time, '▶'), url: vUrl, picture: img, img: img }); 
                    } 
                } 
                return res; 
            },
            ...
            getSort: function(doc, curUrl) { 
                var path = curUrl.replace(this.domain, '').split('?')[0], active = 'New'; 
                if (path.indexOf('most-viewed') !== -1) active = 'Most Viewed 30 Days'; else if (path.indexOf('top-porn') !== -1) active = 'Top Rated 30 Days'; 
                var items = []; items.push({ title: '⇅ ' + active, action: 'none' }); 
                if (active !== 'New') items.push({ title: 'New', url: this.domain + '/' }); 
                if (active !== 'Most Viewed 30 Days') items.push({ title: 'Most Viewed 30 Days', url: this.domain + '/top-50-most-viewed-videos.html' }); 
                if (active !== 'Top Rated 30 Days') items.push({ title: 'Top Rated 30 Days', url: this.domain + '/top-porn-videos.html' }); 
                return { active: active, items: items }; 
            },
            getMenu: function(doc) { 
                var m = [], fmetaBlock = doc.querySelector('.fmeta'); 
                if (fmetaBlock) { 
                    ...
                    for (var ym = 0; ym < ypModels.length; ym++) { 
                        ...
                        if (mName && addedYP.indexOf(mName) === -1) { m.push({ title: mName, action: 'direct', url: mHref }); addedYP.push(mName); } 
                    } 
                } 
                m.push({ title: 'Категорії', action: 'cats_custom', sel: '.full-tags a' }, { title: 'Схожі відео', action: 'sim' }); 
                return m; 
            },
            play: function(doc, el) { 
                var str = [], ypSources = doc.querySelectorAll('video source'); 
                for(var y=0; y<ypSources.length; y++) { 
                    var sUrl = ypSources[y].getAttribute('src'); 
                    if (sUrl) str.push({ title: 'Оригінал', url: sUrl }); 
                } 
                if (str.length > 0) { 
                    var pData = { title: el.name, url: str[0].url, quality: str, headers: { 'Referer': this.domain+'/', 'Origin': this.domain } }; 
                    Lampa.Player.play(pData); Lampa.Player.playlist([pData]); 
        ...
        Adapters['pornmz'] = {
            ...
            parse: function(doc, obj) {
                ...
                for (var i = 0; i < elements.length; i++) {
                    ...
                    if (linkEl) {
                        var vUrl = linkEl.getAttribute('href'); 
                        if (vUrl && vUrl.indexOf('http') !== 0) vUrl = this.domain + (vUrl.startsWith('/') ? '' : '/') + vUrl.replace(/^\//, '');
                        var img = el.getAttribute('data-main-thumb') || ''; 
                        if (!img) { var imgEl = el.querySelector('img'); if(imgEl) img = imgEl.getAttribute('data-src') || imgEl.getAttribute('src') || ''; }
                        if (img && img.indexOf('//') === 0) img = 'https:' + img; else if (img && img.indexOf('/') === 0) img = this.domain + img;
            ...
            getFilter: function() { return [ { title: '🗄️ Категорії', action: 'categories' }, { title: '👸 Моделі', action: 'models' } ]; },
            getSort: function(doc) { 
                var active = 'Newest', items = [], list = doc.querySelector('.filters-list'); 
                if (list) { 
                    var actA = list.querySelector('a.active'); 
                    if (actA) active = actA.textContent.trim(); 
                    var links = list.querySelectorAll('a'); 
                    for (var i = 0; i < links.length; i++) { 
                        var t = links[i].textContent.trim(), h = links[i].getAttribute('href'); 
                        if (h && h.indexOf('http') !== 0) h = this.domain + (h.startsWith('/')?'':'/') + h.replace(/^\//, ''); 
                        if (t === active) items.push({ title: '⇅ ' + t, action: 'none' }); else items.push({ title: t, url: h }); 
                    } 
                } 
                return { active: active, items: items }; 
            },
            getMenu: function(doc) { 
                var m = [], mzModels = doc.querySelectorAll('.tags-list a[href*="pmactor"]'); 
                for (var zm = 0; zm < mzModels.length; zm++) { 
                    var mdlTitle = mzModels[zm].textContent.trim(); 
                    if (mdlTitle) m.push({ title: mdlTitle, action: 'direct', url: mzModels[zm].getAttribute('href') }); 
                } 
                m.push({ title: 'Категорії', action: 'cats_custom', sel: '.tags-list a[href*="/c/"]' }, { title: 'Схожі відео', action: 'sim' }); 
                return m; 
            },
            play: function(doc, el) { 
                ...
                if (vSrc && vSrc.indexOf('.m3u8') !== -1) {
                    smartRequest(vSrc, function(m3Text) {
                        ...
                        var pData = { title: el.name, url: bestUrl || vSrc, headers: { 'Referer': PORNMZ_DOMAIN+'/', 'Origin': PORNMZ_DOMAIN } }; 
                        Lampa.Player.play(pData); Lampa.Player.playlist([pData]);
                    }, function() { 
                        var pData2 = { title: el.name, url: vSrc, headers: { 'Referer': PORNMZ_DOMAIN+'/', 'Origin': PORNMZ_DOMAIN } }; 
                        Lampa.Player.play(pData2); Lampa.Player.playlist([pData2]); 
                    });
                } else if (vSrc) { 
                    Lampa.Player.play({ title: el.name, url: vSrc }); Lampa.Player.playlist([{ title: el.name, url: vSrc }]); 
                } else { 
                    var pzSources = doc.querySelectorAll('video source'), str = []; 
                    for(var z=0; z<pzSources.length; z++) { 
                        var zUrl = pzSources[z].getAttribute('src'); 
                        if (zUrl) str.push({ title: pzSources[z].getAttribute('label') || 'Оригінал', url: zUrl }); 
                    } 
                    if (str.length > 0) { 
                        var pData3 = { title: el.name, url: str[0].url, quality: str };
                        Lampa.Player.play(pData3); Lampa.Player.playlist([pData3]); 
                    } 
                }
            },
            checkEnd: function(doc, url, page) { 
                var canon = doc.querySelector('link[rel="canonical"]'); 
                if (page > 1 && canon && canon.href && url.indexOf('/page/' + page) !== -1 && canon.href.indexOf('/page/' + page) === -1) return true; 
                var pagination = doc.querySelector('.pagination'); 
                if (pagination) { 
                    var currentSpan = pagination.querySelector('.current'); 
                    if (!currentSpan || parseInt(currentSpan.textContent) !== page) return true; 
        ...
        function CustomCatalog(object) {
            ...
            comp.create = function () {
                ...
                if (currentSite === 'bookmarks') { 
                    ...
                    if (bmarks.length > 0) { 
                        ...
                        if (bmarks[0].is_porndish_list) rendered.addClass('is-porndish-list'); 
                        else if (bmarks[0].is_16x9_grid) rendered.addClass('is-16x9-grid'); 
                        else if (bmarks[0].is_studios_noimg) rendered.addClass('is-noimg-grid'); 
                ...
                // Формування URL з пагінацією
                if (currentSite === 'lenkino' || currentSite === 'youperv' || currentSite === 'pornmz') { 
                    var uParts = target.split('?'), baseS = uParts[0].replace(/\/page\/[0-9]+(\/?)$/, '').replace(/\/+$/, ''); 
                    if(baseS.indexOf('.html') === -1 && target.indexOf('do=search') === -1) target = baseS + (object.page > 1 ? '/page/' + object.page + '/' : '/') + (uParts.length > 1 ? '?' + uParts[1] : ''); 
                } else if ((currentSite === 'longvideos' || currentSite === 'porndish') && object.page > 1) { 
                    var uParts2 = target.split('?'), baseS2 = uParts2[0].replace(/\/page\/[0-9]+\/?$/, '').replace(/\/+$/, ''); 
                    if (!baseS2.endsWith('/')) baseS2 += '/'; target = baseS2 + 'page/' + object.page + '/' + (uParts2.length > 1 ? '?' + uParts2[1] : ''); 
            ...
            comp.nextPageReuest = function (object, resolve, reject) {
                ...
                if (currentSite === 'lenkino' || currentSite === 'youperv' || currentSite === 'pornmz') { 
                    var uP = url.split('?'), bL = uP[0].replace(/\/page\/[0-9]+(\/?)$/, '').replace(/\/+$/, ''); 
                    if(bL.indexOf('.html') === -1 && url.indexOf('do=search') === -1) url = bL + '/page/' + object.page + '/' + (uP.length > 1 ? '?' + uP[1] : ''); 
                } else if (currentSite === 'longvideos' || currentSite === 'porndish') { 
                    var uP2 = url.split('?'), bL2 = uP2[0].replace(/\/page\/[0-9]+\/?$/, '').replace(/\/+$/, ''); 
                    if (!bL2.endsWith('/')) bL2 += '/'; url = bL2 + 'page/' + object.page + '/' + (uP2.length > 1 ? '?' + uP2[1] : ''); 
                } else if (currentSite === 'pornhub') { 
                    url = (object.url || (PORNHUB_DOMAIN + '/video')) + ((object.url || '').indexOf('?') === -1 ? '?' : '&') + 'page=' + object.page; 
                } else { 
                    var base365 = (adapter.domain).replace(/\/page\/[0-9]+$/, '').replace(/\/+$/, ''); 
                ...
                Lampa.Select.show({ title: 'Навігація', items: items, onSelect: function (a) {
                    ...
                    if (a.action === 'home') { Lampa.Activity.push({ url: adapter.domain + (currentSite==='longvideos'?'/latest-updates/':(currentSite==='pornhub'?'/video':'/')), title: 'Головна', component: 'pluginx_comp', site: currentSite, page: 1 }); }
                        ...
                        Lampa.Input.edit({ title: 'Пошук', value: '', free: true, nosave: true }, function(v) { 
                            if (v) { 
                                var sUrl = adapter.domain + '/search/?q=' + encodeURIComponent(v); 
                                if (currentSite === 'lenkino') sUrl = adapter.domain + '/search/' + encodeURIComponent(v); 
                                if (currentSite === 'longvideos') sUrl = adapter.domain + '/search/' + encodeURIComponent(v) + '/relevance/'; 
                                if (currentSite === 'pornhub') sUrl = adapter.domain + '/video/search?search=' + encodeURIComponent(v); 
                                if (currentSite === 'porndish' || currentSite === 'pornmz') sUrl = adapter.domain + '/?s=' + encodeURIComponent(v); 
                                if (currentSite === 'youperv') { 
                                    if(v.length < 4) { Lampa.Noty.show('Мінімум 4 символи для YouPerv'); return; } 
                                    sUrl = adapter.domain + '/index.php?do=search&subaction=search&story=' + encodeURIComponent(v); 
                                } 
                    ...
                    else if (a.action === 'yp_pop_cats') { 
                        smartRequest(adapter.domain + '/', function(html) { 
                            var doc = new DOMParser().parseFromString(html, 'text/html'), links = doc.querySelectorAll('span[class^="clouds_"] a'), menu = [], added = []; 
                            for(var i=0; i<links.length; i++) { 
                                var h = links[i].getAttribute('href'), t = (links[i].textContent || '').trim(); 
                                if(h && t && added.indexOf(t) === -1) { menu.push({title: t, url: h.startsWith('http') ? h : adapter.domain + h}); added.push(t); } 
                            } 
                            if(menu.length) Lampa.Select.show({ title: '🗄️ Популярні категорії', items: menu, onSelect: function(it) { Lampa.Activity.push({ url: it.url, title: it.title, component: 'pluginx_comp', site: currentSite, page: 1 }); }, onBack: function() { comp.filter(); } }); 
                        }); 
                    }
                    else if (a.action === 'lv_cat_list' || a.action === 'lv_trend_list') { 
                        smartRequest(adapter.domain + '/categories/', function(html) { 
                            var doc = new DOMParser().parseFromString(html, 'text/html'), links = doc.querySelectorAll(a.action === 'lv_trend_list' ? '.tags__item' : '.list-categories__row--list a'), menu = []; 
                            for(var i=0; i<links.length; i++) { 
                                var h = links[i].getAttribute('href'); 
                                if(h) menu.push({title: (links[i].textContent || '').trim(), url: h.startsWith('http') ? h : adapter.domain + h}); 
                            } 
                            if(menu.length) Lampa.Select.show({ title: a.action === 'lv_trend_list' ? '🔥 Трендові запити' : '🗄️ Категорії', items: menu, onSelect: function(it) { Lampa.Activity.push({ url: it.url, title: it.title, component: 'pluginx_comp', site: currentSite, page: 1 }); }, onBack: function() { comp.filter(); } }); 
                        }); 
                    }
                    else if (a.action === 'ph_cat_list') { 
                        var mPH = []; 
                        for(var c=0; c<PH_CATS.length; c++) mPH.push({title: PH_CATS[c].title, url: adapter.domain + '/video' + (PH_CATS[c].val ? '?c=' + PH_CATS[c].val : '') }); 
                        Lampa.Select.show({ title: '🗄️ Категорії', items: mPH, onSelect: function(it) { Lampa.Activity.push({ url: it.url, title: it.title, component: 'pluginx_comp', site: currentSite, page: 1 }); }, onBack: function() { comp.filter(); } }); 
                    }
                    else if (a.action === 'sort') {
                        Lampa.Select.show({ title: 'Сортування', items: comp._sortData.items, onSelect: function(s) { 
            ...
            comp.cardRender = function (card, element, events) {
                events.onEnter = function () {
                    ...
                    if (currentSite === 'bookmarks') { 
                        ...
                        else if (element.url.indexOf(PORNDISH_DOMAIN) !== -1) targetSite = 'porndish'; 
                        else if (element.url.indexOf(YOUPERV_DOMAIN) !== -1) targetSite = 'youperv'; 
                        else if (element.url.indexOf(PORNMZ_DOMAIN) !== -1) targetSite = 'pornmz'; 
                ...
                events.onMenu = function () {
                    ...
                    smartRequest(element.url, function (htmlText) {
                        var doc = new DOMParser().parseFromString(htmlText, 'text/html');
                        var menu = [{ title: isBookmarked ? '★ Видалити з обраного' : '☆ Додати до обраного', action: 'bookmark' }];
                        var adapter = Adapters[targetSite] || Adapters['porno365'];
        ...
        (function() { 
            ...
            filterBtn.hide().on('hover:enter click', function() { 
                if (isClicking) return; isClicking = true; 
                setTimeout(function() { isClicking = false; }, 300); 
                try { 
                    if (currentActivity && currentActivity.activity) { 
                        var c = (window.Lampa.Manifest && window.Lampa.Manifest.app_digital >= 300) ? currentActivity.activity.component : currentActivity.activity.component(); 
            ...
            Lampa.Listener.follow('activity', function(e) { 
                ...
                if (e.type == 'start' && e.component == 'pluginx_comp') { 
                    ...
                    else $('.head__actions').prepend(filterBtn); filterBtn.show(); currentActivity = e.object; 
                } 
            }); 
        })();
    }
            ...
            item.on('hover:enter', function () { 
                Lampa.Select.show({ 
                    ...
                    items: [
                        ...
                        { title: 'LongVideos', site: 'longvideos' }, 
                        { title: 'Pornhub', site: 'pornhub' }, 
                        { title: 'Porndish', site: 'porndish' }, 
                        { title: 'YouPerv', site: 'youperv' }, 
                        { title: 'Pornmz', site: 'pornmz' }
                    ], 
                    onSelect: function(a) { Lampa.Activity.push({ title: a.title, component: 'pluginx_comp', site: a.site, page: 1 }); }, 
