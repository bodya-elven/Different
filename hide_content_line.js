...
(function () {
    ...
    // Керування чорним списком + Розумна передача фокусу
    function toggleBlacklist(cardData) {
        ...
        if (isBlocked) {
            ...
            if (active && active.activity && active.activity.render) {
                ...
                if (focusEl.length) {
                    // Шукаємо сусідню видиму картку (справа, або якщо немає - зліва)
                    var next = focusEl.nextAll('.item:visible').first();
                    if (!next.length) next = focusEl.prevAll('.item:visible').first();

                    // Повністю видаляємо картку з екрана
                    focusEl.remove();

                    // Оновлюємо контролер Лампи і передаємо фокус сусіду
    ...
    var hideProcessor = {
        filters: [
            ...
            function (items) {
                ...
                return items.filter(function (item) {
                    if (!isMediaContent(item)) return true;
                    if (!item) return true;
                    // Безпечна перевірка на наявність текстового опису
        ...
        apply: function (data) {
            var results = data.slice(); // Нативне безпечне клонування масиву
    ...
    function initPlugin() {
        ...
        // Нативна фільтрація через хук Lampa.Maker (Сучасний метод автора Лампи)
        var lineMaker = null;
        if (Lampa.Maker) {
            // Захист від різних версій: перевіряємо чи map це функція або об'єкт
            if (typeof Lampa.Maker.map === 'function') lineMaker = Lampa.Maker.map('Line');
            else if (typeof Lampa.Maker.map === 'object') lineMaker = Lampa.Maker.map['Line'];
        }

        if (lineMaker && lineMaker.Create) {
            var origLineInit = lineMaker.Create.onInit;
            lineMaker.Create.onInit = function () {
                if (this.data && Array.isArray(this.data.results) && !this.data._filtered_by_hiding) {
                    this.data._filtered_by_hiding = true; // Ставимо мітку, щоб не фільтрувати двічі
                    if (this.data.results.some(isMediaContent)) {
                        this.data.original_length = this.data.results.length;
                        this.data.results = hideProcessor.apply(this.data.results);
                    }
                }
                if (typeof origLineInit === 'function') {
                    return origLineInit.apply(this, arguments);
                }
            };
        }

        // Страховка (Fallback) для повних категорій, які можуть не використовувати Line Maker
        Lampa.Listener.follow('request_secuses', function (e) {
            if (!e.data || !Array.isArray(e.data.results) || e.data._filtered_by_hiding) return;
            ...
            e.data._filtered_by_hiding = true; // Ставимо мітку
        ...
        // Кнопка "Ще" на випадок занадто сильного приховування в рядках
